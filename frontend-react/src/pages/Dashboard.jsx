...
const PLAN_COLORS = {
    FREE: { badge: 'bg-gray-500/15 border-gray-500/30 text-gray-600 dark:text-gray-400', bar: 'bg-gray-500' },
...
function CopyBtn({ text, className = '' }) {
    ...
    return (
        <button onClick={copy} className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-black/[0.04] dark:bg-white/[0.04] hover:bg-black/10 dark:bg-white/10 border border-gray-200 dark:border-white/10 text-xs text-gray-600 dark:text-gray-400 hover:text-gray-900 dark:text-white transition-all ${className}`}>
    ...
    return (
        <div>
            <div className="flex items-center justify-between text-xs mb-1.5">
                ...
                <span className="text-gray-700 dark:text-gray-300 font-mono">{value}<span className="text-gray-600"> / {max}</span></span>
            </div>
            ...
            {days.map((d) => {
                ...
                return (
                    <div key={d.day} className="flex-1 flex flex-col items-center gap-1 group relative">
                        <div className="absolute -top-7 left-1/2 -translate-x-1/2 hidden group-hover:flex items-center bg-gray-900 border border-gray-200 dark:border-white/10 rounded px-2 py-0.5 text-xs text-gray-900 dark:text-white whitespace-nowrap z-10">
        ...
        <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-12 relative">
            ...
            <AnimatePresence>
                {toast && (
                    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 20 }}
                        className="fixed bottom-6 right-6 z-50 bg-gray-900 border border-gray-200 dark:border-white/10 rounded-xl px-4 py-3 text-sm text-gray-900 dark:text-white shadow-xl">
                        {toast}
                    </motion.div>
                )}
            </AnimatePresence>
            ...
            <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} className="flex items-center justify-between mb-10 gap-4 flex-wrap">
                <div className="flex items-center gap-4">
                    <div className="w-12 h-12 rounded-xl bg-gradient-to-br from-violet-600 to-cyan-600 flex items-center justify-center text-gray-900 dark:text-white font-black text-lg shadow-lg shadow-violet-500/20">
                        {initials}
                    </div>
                    <div>
                        <h1 className="text-xl font-black text-gray-900 dark:text-white">Welcome back, {user?.name?.split(' ')[0] || 'Developer'}!</h1>
                ...
                <div className="flex items-center gap-3">
                    ...
                    <button onClick={handleLogout} className="flex items-center gap-2 px-4 py-2 bg-black/[0.04] dark:bg-white/[0.04] border border-gray-200 dark:border-white/10 text-gray-600 dark:text-gray-400 text-sm rounded-xl hover:bg-red-500/10 hover:border-red-500/20 hover:text-red-400 transition-all">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <div className="lg:col-span-2 space-y-6">
                    ...
                    <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }}
                        className="bg-white dark:bg-[#0d1117] border border-gray-200 dark:border-white/10 rounded-2xl p-6">
                        <div className="flex items-center justify-between mb-4">
                            <div className="flex items-center gap-2.5">
                                ...
                                <div>
                                    <h2 className="font-bold text-gray-900 dark:text-white text-sm">API Key</h2>
                                    <p className="text-xs text-gray-600">Use this as your Bearer token</p>
                                </div>
                            </div>
                            <div className="flex items-center gap-2">
                                <button onClick={handleReveal} disabled={revealing}
                                    className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-black/[0.04] dark:bg-white/[0.04] hover:bg-black/10 dark:bg-white/10 border border-gray-200 dark:border-white/10 text-xs text-gray-600 dark:text-gray-400 hover:text-gray-900 dark:text-white transition-all">
                                    {revealing ? <Loader2 className="w-3 h-3 animate-spin" /> : showKey ? <EyeOff className="w-3 h-3" /> : <Eye className="w-3 h-3" />}
                                    {showKey ? 'Hide' : 'Reveal'}
                                </button>
                                <button onClick={handleRegenerate} disabled={regenerating}
                                    className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-black/[0.04] dark:bg-white/[0.04] hover:bg-red-500/10 border border-gray-200 dark:border-white/10 hover:border-red-500/20 text-xs text-gray-600 dark:text-gray-400 hover:text-red-400 transition-all">
                    ...
                    <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.15 }}
                        ...
                        <div className="flex items-center justify-between mb-5">
                            <div className="flex items-center gap-2.5">
                                ...
                                <h2 className="font-bold text-gray-900 dark:text-white text-sm">API Calls — Last 7 Days</h2>
                            </div>
                    ...
                    <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }}
                        className="bg-white dark:bg-[#0d1117] border border-gray-200 dark:border-white/10 rounded-2xl overflow-hidden">
                        <div className="flex items-center justify-between px-6 py-4 border-b border-white/[0.06]">
                            <div className="flex items-center gap-2.5">
                                <div className="p-2 rounded-lg bg-emerald-500/10 border border-emerald-500/20">
                                    <Terminal className="w-4 h-4 text-emerald-400" />
                                </div>
                                <h2 className="font-bold text-gray-900 dark:text-white text-sm">Quick Start</h2>
                            </div>
                            <div className="flex gap-1">
                                {['curl', 'javascript', 'python'].map(l => (
                                    <button key={l} onClick={() => setCodeLang(l)}
                                        className={`px-3 py-1.5 text-xs font-medium rounded-lg capitalize transition-all ${codeLang === l ? 'bg-black/10 dark:bg-white/10 text-gray-900 dark:text-white' : 'text-gray-600 hover:text-gray-700 dark:text-gray-300'}`}>
                                        {l === 'javascript' ? 'JS' : l}
                                    </button>
                                ))}
                            </div>
                        ...
                        <div className="relative">
                            <pre className="p-5 text-xs font-mono text-gray-700 dark:text-gray-300 overflow-x-auto leading-7 whitespace-pre">
                ...
                <div className="space-y-6">
                    ...
                    {/* Stats cards */}
                    <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }}
                        className="bg-white dark:bg-[#0d1117] border border-gray-200 dark:border-white/10 rounded-2xl p-6 space-y-4">
                        <div className="flex items-center gap-2 mb-2">
                            <div className="p-2 rounded-lg bg-yellow-500/10 border border-yellow-500/20">
                                <Activity className="w-4 h-4 text-yellow-400" />
                            </div>
                            <h2 className="font-bold text-gray-900 dark:text-white text-sm">Usage</h2>
                        </div>
                        ...
                        <div className="grid grid-cols-2 gap-3">
                            {[
                                ...
                                <div key={s.label} className="bg-black/[0.03] dark:bg-white/[0.03] border border-white/[0.06] rounded-xl p-3">
                    ...
                    <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.15 }}
                        className="bg-white dark:bg-[#0d1117] border border-gray-200 dark:border-white/10 rounded-2xl p-6">
                        <div className="flex items-center justify-between mb-4">
                            <h2 className="font-bold text-gray-900 dark:text-white text-sm">Current Plan</h2>
                            <span className={`text-xs font-bold uppercase tracking-wider px-2.5 py-1 rounded-lg border ${planStyle.badge}`}>{plan}</span>
                        </div>
                        <div className="space-y-2 text-xs text-gray-500 dark:text-gray-500 mb-5">
                            <div className="flex justify-between"><span>Rate limit</span><span className="text-gray-700 dark:text-gray-300">{rateLimit} req/min</span></div>
                        ...
                        {plan === 'FREE' && (
                            <Link to="/pricing"
                                className="block w-full py-2.5 rounded-xl bg-gradient-to-r from-violet-600 to-cyan-600 text-gray-900 dark:text-white text-sm font-bold text-center hover:scale-[1.02] transition-transform">
                                Upgrade to Pro →
                            </Link>
                        )}
                    </motion.div>
                    ...
                    {/* Quick links */}
                    <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }}
                        className="bg-white dark:bg-[#0d1117] border border-gray-200 dark:border-white/10 rounded-2xl p-5">
                        <p className="text-xs font-bold text-gray-500 dark:text-gray-500 uppercase tracking-wider mb-3">Quick Links</p>
                        <div className="space-y-0.5">
                            {[
                                ...
                                <Link key={l.to} to={l.to} className="flex items-center gap-3 px-3 py-2.5 rounded-xl hover:bg-black/[0.04] dark:bg-white/[0.04] transition-colors group">
                                    <l.icon className="w-4 h-4 text-gray-600 group-hover:text-violet-400 transition-colors" />
                                    <div>
                                        <p className="text-sm text-gray-700 dark:text-gray-300 group-hover:text-gray-900 dark:text-white transition-colors">{l.label}</p>
                                        <p className="text-xs text-gray-600">{l.sub}</p>
                                    </div>
                                    <ArrowRight className="w-3.5 h-3.5 text-gray-700 ml-auto group-hover:text-gray-600 dark:text-gray-400 group-hover:translate-x-0.5 transition-all" />
                                </Link>
                            ))}
                        </div>
                    </motion.div>
