            ...
            <div className="flex border-b border-white/10 bg-white/[0.03]">
                {tabs.map((t, i) => (
                    <button
                        key={t}
                        onClick={() => setActive(i)}
                        className={`px-5 py-3 text-sm font-medium transition-all ${active === i ? 'text-violet-400 border-b-2 border-violet-500' : 'text-gray-500 hover:text-gray-300'}`}
                    >
                        {t}
                    </button>
                ))}
            </div>
            <pre className="p-6 overflow-x-auto text-sm leading-7 text-gray-300 font-mono whitespace-pre">
...
export default function Landing() {
    return (
        <div className="relative overflow-x-hidden">
            ...
            <section className="relative pt-28 md:pt-36 pb-24 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto text-center">
                <motion.div initial="hidden" animate="visible" variants={stagger}>
                    <motion.div variants={fadeUp} className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-violet-500/10 border border-violet-500/20 mb-8">
                        ...
                        <span className="text-sm font-medium text-gray-300">v2.0 Visual Builder Now Live</span>
                        <ChevronRight className="w-3.5 h-3.5 text-gray-500" />
                    </motion.div>

                    <motion.h1 variants={fadeUp} className="text-5xl md:text-7xl font-extrabold tracking-tight mb-6 leading-[1.1]">
                        <span className="text-white">Developer-First</span><br />
                    ...
                    <motion.div variants={fadeUp} className="flex flex-col sm:flex-row justify-center items-center gap-4">
                        ...
                        <Link to="/docs" className="flex items-center gap-2 px-8 py-4 bg-white/5 border border-white/10 hover:bg-white/10 rounded-xl font-semibold text-lg transition-all backdrop-blur-md">
                ...
                <Section className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    {FEATURES.map((f, i) => (
                        <motion.div key={i} variants={fadeUp} className="group relative p-px rounded-2xl bg-gradient-to-b from-white/10 to-transparent hover:from-violet-500/30 hover:to-cyan-500/10 transition-all duration-500">
                            <div className="h-full bg-[#0d1117] border border-white/5 group-hover:border-white/10 rounded-2xl p-6 transition-all">
                                <div className={`w-11 h-11 rounded-xl border flex items-center justify-center mb-4 ${f.iconBg}`}>
                                    {f.icon}
                                </div>
                                <div className="flex items-center justify-between mb-2">
                                    <h3 className="text-base font-bold text-white">{f.title}</h3>
                                    <span className="text-xs bg-white/5 border border-white/10 text-gray-400 px-2 py-0.5 rounded-full font-mono">{f.badge}</span>
                                </div>
            ...
            <section className="px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto pb-24">
                <Section>
                    <motion.div variants={fadeUp} className="text-center mb-14">
                        ...
                        <h2 className="text-3xl md:text-4xl font-extrabold text-white mt-3">Three steps to your chart</h2>
                        <p className="text-gray-400 mt-3 max-w-lg mx-auto">No complex setup. No SDK needed. Just a URL that returns an image.</p>
                    </motion.div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-8 relative">
                        ...
                        {HOW_IT_WORKS.map((step, i) => (
                            <motion.div key={i} variants={fadeUp} className="relative flex flex-col items-center text-center p-6 rounded-2xl bg-white/[0.03] border border-white/8">
                                <div className={`w-16 h-16 rounded-full flex items-center justify-center bg-gradient-to-br ${step.color} text-white font-black text-xl mb-6 shadow-lg`}>
                                    {step.step}
                                </div>
            ...
            <section className="px-4 sm:px-6 lg:px-8 max-w-5xl mx-auto pb-24">
                <Section>
                    <motion.div variants={fadeUp} className="text-center mb-10">
                        ...
                        <h2 className="text-3xl md:text-4xl font-extrabold text-white mt-3">One request, one image</h2>
                        <p className="text-gray-400 mt-3 max-w-lg mx-auto">The simplest API you've ever used. A GET request that responds with a PNG.</p>
                    </motion.div>
            ...
            <section className="px-4 sm:px-6 lg:px-8 max-w-5xl mx-auto pb-24">
                <Section>
                    <motion.div variants={fadeUp} className="rounded-2xl border border-white/10 bg-white/[0.03] overflow-hidden">
                        <div className="p-6 border-b border-white/10">
                            <span className="text-xs font-bold uppercase tracking-widest text-emerald-400">API Surface</span>
                            <h2 className="text-2xl font-bold text-white mt-2">Available Endpoints</h2>
                        </div>
            ...
            <section className="px-4 sm:px-6 lg:px-8 max-w-5xl mx-auto pb-32">
                <Section>
                    <motion.div variants={fadeUp} className="relative rounded-3xl overflow-hidden p-10 text-center">
                        <div className="absolute inset-0 bg-gradient-to-br from-violet-600/20 via-fuchsia-600/10 to-cyan-600/20 backdrop-blur-sm border border-white/10 rounded-3xl" />
                        <div className="relative">
                            <h2 className="text-3xl md:text-4xl font-extrabold text-white mb-4">Ready to build?</h2>
                            <p className="text-gray-400 mb-8 max-w-md mx-auto">Open the Visual Builder, configure your chart, and get your API URL in seconds.</p>
                            <Link to="/builder">
                                <button className="inline-flex items-center gap-2 px-8 py-4 bg-gradient-to-r from-violet-600 to-cyan-600 rounded-xl font-bold text-white text-lg hover:scale-105 transition-transform shadow-2xl shadow-violet-500/20">
                                    Open Visual Builder
